...
export default function StatCard({ title, amount, trend, icon: Icon, variant = 'primary' }) {
  ...
  return (
    <motion.div 
      ...
      className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/50 group"
    >
      ...
      <div>
        ...
        <h3 className="text-2xl font-bold text-slate-900 tracking-tight">{formattedAmount}</h3>
      </div>
    </motion.div>
  );
}
